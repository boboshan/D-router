...
namespace dcr
{
    ...
    void StatusPanel::refresh()
    {
        ...
        else
        {
            const uint64_t dP = processedBlocks > prevProcessedBlocks
                                    ? processedBlocks - prevProcessedBlocks
                                    : 0;
            const uint64_t dS = stalledBlocks > prevStalledBlocks
                                    ? stalledBlocks - prevStalledBlocks
                                    : 0;
    ...
    void StatusPanel::GaugeStrip::paint (juce::Graphics& g)
    {
        ...
        auto drawCard = [&] (juce::Rectangle<int> r,
            ...
            g.drawText (title, inner.removeFromTop (12).withTrimmedRight (16), juce::Justification::topLeft);
            ...
            if (fillRatio >= 0.0f)
            {
                ...
                g.fillRect (barArea);
                const int filledW = juce::jlimit (0, barArea.getWidth(), (int) std::round (fillRatio * (float) barArea.getWidth()));
                g.setColour (valueCol);
            ...
            g.drawText ("healthy: " + healthyHint, inner.removeFromTop (12), juce::Justification::topLeft);
        };
        ...
        //    5 ms is risky regardless of how big your ring is.
        {
            ...
            if (owner.lastOutRingMinMs < 3.0)
                ringCol = critical;
            else if (owner.lastOutRingMinMs < 8.0)
                ringCol = warning;
        ...
        //    the device callback lags behind.
        {
            ...
            if (owner.lastPollsPerBlock > 50.0f)
                pollCol = critical;
            else if (owner.lastPollsPerBlock > 25.0f)
                pollCol = warning;
            drawCard (r,
        ...
        // 5. Last dropout
        {
            ...
            juce::String valText;
            if (owner.lastDropoutAgoSec < 0.0)
                valText = "never";
            else if (owner.lastDropoutAgoSec < 60.0)
                valText = juce::String (owner.lastDropoutAgoSec, 1) + " s";
            else
                valText = juce::String ((int) (owner.lastDropoutAgoSec / 60.0)) + "m";
            drawCard (r,
                "LAST DROPOUT",
                valText,
                "never",
