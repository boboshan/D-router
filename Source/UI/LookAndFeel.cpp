...
namespace dcr
{
    ...
    void LookAndFeel::drawButtonBackground (juce::Graphics& g, juce::Button& button, const juce::Colour& /*backgroundColour*/, bool shouldDrawButtonAsHighlighted, bool shouldDrawButtonAsDown)
    {
        ...
        else if (button.getName() == "fx")
        {
            ...
            if (shouldDrawButtonAsDown)
                baseColor = baseColor.darker (0.30f);
    ...
    void LookAndFeel::drawRotarySlider (juce::Graphics& g, int x, int y, int width, int height, float sliderPosProportional, float rotaryStartAngle, float rotaryEndAngle, juce::Slider& slider)
    {
        ...
        juce::Path arc;
        arc.addCentredArc (centreX, centreY, radius - 2.5f, radius - 2.5f, 0.0f, rotaryStartAngle, angle, true);
    ...
    void LookAndFeel::drawLinearSlider (juce::Graphics& g, int x, int y, int width, int height, float sliderPos, float /*minSliderPos*/, float /*maxSliderPos*/, const juce::Slider::SliderStyle style, juce::Slider& slider)
    {
        ...
        if (isVertical)
        {
            ...
            g.drawLine (handleX + 1.0f, handleY + handleH * 0.5f, handleX + handleW - 1.0f, handleY + handleH * 0.5f, 1.5f);
        }
        else
        {
            ...
            g.setColour (slider.isEnabled() ? accentColor : juce::Colours::grey);
            g.drawLine (handleX + handleW * 0.5f, handleY + 1.0f, handleX + handleW * 0.5f, handleY + handleH - 1.0f, 1.5f);
        }
    }
    ...
    void LookAndFeel::drawPopupMenuItem (juce::Graphics& g, const juce::Rectangle<int>& area, bool isSeparator, bool isActive, bool isHighlighted, bool isChecked, bool /*hasSubMenu*/, const juce::String& text, const juce::String& shortcutKeyText, const juce::Drawable* /*icon*/, const juce::Colour* textColour)
    {
        if (isSeparator)
        {
            g.setColour (juce::Colour::fromRGB (28, 28, 34));
    ...
    void LookAndFeel::drawScrollbarButton (juce::Graphics& /*g*/, juce::ScrollBar& /*scrollbar*/, int /*width*/, int /*height*/, int /*buttonDirection*/, bool /*isScrollbarVertical*/, bool /*isMouseOverButton*/, bool /*isButtonDown*/)
    {
        // No arrows on scrollbars for a clean modern flat style
    }

    void LookAndFeel::drawScrollbar (juce::Graphics& g, juce::ScrollBar& scrollbar, int x, int y, int width, int height, bool isScrollbarVertical, int thumbPosition, int thumbSize, bool isMouseOver, bool /*isMouseDown*/)
    {
