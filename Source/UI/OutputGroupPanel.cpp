...
namespace dcr
{
    ...
    juce::String OutputGroupPanel::resolveChannelName (int globalCh) const
    {
        ...
        for (auto& d : engine.getDeviceInfo())
        {
            ...
            for (int ci = 0; ci < n; ++ci, ++idx)
                if (idx == globalCh)
                    return d.name + " " + juce::String (ci + 1);
        }
        return "?";
    }
    ...
    MultiChannelPluginHost* OutputGroupPanel::Card::getSlotHost (int slotIdx) const
    {
        ...
        if (g == nullptr)
            return nullptr;
        if (slotIdx < 0 || slotIdx >= (int) g->pluginSlots.size())
            return nullptr;
    ...
    {
        auto* host = getSlotHost (slotIdx);
        if (host && host->getPlugin())
            openEditorFor (slotIdx);
        else
            panel->requestLoadPlugin (groupIdx, slotIdx);
    }

    void OutputGroupPanel::Card::onSlotRemoveClicked (int slotIdx)
    {
        closeEditorFor (slotIdx);
        if (auto* host = getSlotHost (slotIdx))
            host->clearPlugin();
    ...
    void OutputGroupPanel::timerCallback()
    {
        ...
        for (int i = 0; i < cards.size(); ++i)
        {
            ...
            if (hoverIdx < 0 && cards[i]->isMouseOver (true))
                hoverIdx = i;
        }
    ...
    void OutputGroupPanel::installPluginIntoGroup (int cardIdx, int slotIdx, juce::PluginDescription desc)
    {
        engine.getPluginFormatManager().createPluginInstanceAsync (
            desc, engine.getEngineSampleRate(), engine.getEngineBlockSize(), [this, cardIdx, slotIdx] (std::unique_ptr<juce::AudioPluginInstance> instance, const juce::String& error) {
                if (instance == nullptr)
                {
                ...
                Card* card = nullptr;
                for (auto* c : cards)
                    if (c->groupIdx == cardIdx)
                    {
                        card = c;
                        break;
                    }
                if (card != nullptr)
                    card->closeEditorFor (slotIdx);

                host->setPlugin (std::move (instance), g->channelSet);

                if (card != nullptr)
                    card->refreshSlotAppearance (slotIdx);
            });
    }
