...
private:
    void onUpgradeClicked()
    {
        ...
        installer.start (release,
            [safe] (double p)               { if (safe != nullptr) safe->onProgress (p); },
            [safe] (bool ok, juce::String e) { if (safe != nullptr) safe->onDone (ok, e); });
    }
    ...
    void onDone (bool ok, juce::String error)
    {
        ...
        if (error.isEmpty()) return;        // user cancelled -> dialog already closing
