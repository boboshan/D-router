...
void UpdateInstaller::run()
{
    ...
    if (! dir.createDirectory()) { finish (false, "Can't create the update folder."); return; }
    ...
    const auto zip = dir.getChildFile ("D-Router.zip");
    {
        ...
        if (in == nullptr) { finish (false, "Couldn't start the download."); return; }

        juce::FileOutputStream out (zip);
        if (out.failedToOpen()) { finish (false, "Couldn't write the download."); return; }
        ...
        {
            if (cancelled || threadShouldExit()) { zip.deleteFile(); finish (false, {}); return; }
    ...
    staging.createDirectory();
    {
        ...
        unzip.start (juce::StringArray { "/usr/bin/ditto", "-x", "-k",
                                         zip.getFullPathName(), staging.getFullPathName() });
        unzip.waitForProcessToFinish (60000);
        if (unzip.getExitCode() != 0) { finish (false, "Couldn't unpack the update."); return; }
    }

    const auto newApp = staging.getChildFile ("D-Router.app");
    if (! newApp.isDirectory()) { finish (false, "The update package was malformed."); return; }
