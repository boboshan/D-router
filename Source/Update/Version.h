...
inline Version parseVersion (const std::string& s)
{
    ...
    auto readInt = [&] (int& out) -> bool
    {
        ...
        if (any) out = static_cast<int> (val);
        return any;
    };

    if (! readInt (v.major)) return v;                       // need at least a major
    if (i < s.size() && s[i] == '.') { ++i; readInt (v.minor); }
    if (i < s.size() && s[i] == '.') { ++i; readInt (v.patch); }
    if (i < s.size() && s[i] == '-') v.prerelease = s.substr (i + 1);
    v.valid = true;
    return v;
}
...
inline int compareVersions (const Version& a, const Version& b)
{
    ...
    if (ap != bp) return ap ? -1 : 1;                        // has-prerelease is older
...
inline bool isNewer (const std::string& candidate, const std::string& current)
{
    ...
    if (! c.valid || ! r.valid) return false;
