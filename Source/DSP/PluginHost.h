...
namespace dcr
{
    ...
    public:
        ...
        void setPluginAt (int slotIdx, std::unique_ptr<juce::AudioPluginInstance> p, const juce::MemoryBlock* stateToRestore = nullptr);
        ...
        {
            return (slotIdx >= 0 && slotIdx < kNumSlots)
                       ? slots[(size_t) slotIdx].cpuLoadAvg.load (std::memory_order_relaxed)
                       : 0.0f;
        }
        ...
        {
            for (auto const& s : slots)
                if (s.current && !s.bypassed.load (std::memory_order_relaxed))
                    return true;
            return false;
        }
