...
public:
    ...
    {
        for (auto& n : nodes) n.store (0.0f, std::memory_order_relaxed);
    }
    ...
    static APVTS::ParameterLayout createLayout()
    {
        ...
        l.add (std::make_unique<juce::AudioParameterChoice>(
            juce::ParameterID { "mode", 1 }, "Target mode",
            juce::StringArray { "Offset", "Match" }, 0));
        l.add (std::make_unique<juce::AudioParameterChoice>(
            juce::ParameterID { "res", 1 }, "Resolution",
            juce::StringArray { "1/3 oct", "1/6 oct", "1/12 oct", "1/24 oct" }, 1));
...
protected:
    ...
    void computeBinGains (const float* mags, float* gains, int numBins, double /*sr*/, int channel) override
    {
        ...
        if (gen != lastTargetGen) { rebuildTargetFromNodes (numBins); lastTargetGen = gen; }
        ...
        if (mode == 0) logSmooth (magDbWork.data(), refWork.data(), numBins, 1.5f);   // broad trend
        else           levelAnchor = computeLevelAnchor (numBins);
        ...
        if (channel == 0) updateDisplay();
    }

private:
    static int nForRes (int idx) noexcept
    {
        switch (juce::jlimit (0, 3, idx)) { case 0: return 3; case 1: return 6; case 2: return 12; default: return 24; }
    }
    ...
    {
        float nv[kNumNodes];
        for (int i = 0; i < kNumNodes; ++i) nv[i] = nodes[(size_t) i].load (std::memory_order_relaxed);
        for (int k = 0; k < numBins; ++k)
        {
    ...
    void rebuildDisplayBands (int numBins, int N)
    {
        ...
        for (float fc = 20.0f; fc <= fTop && count < kMaxDisp; fc *= step, ++count)
        {
            ...
            dispBinHi [(size_t) count] = juce::jlimit (dispBinLo[(size_t) count], numBins - 1,
                                                       (int) std::ceil (fc * hiMul / binHz));
