...
private:
    ...
    {
        if (auto* a = comp.getValueTreeState().getRawParameterValue (id)) return a->load();
        return 0.0f;
    }
    ...
    void drawMeter (juce::Graphics& g)
    {
        ...
        g.drawText ("GR", meterArea.getX(), meterArea.getBottom() + 1, meterArea.getWidth(), 12,
                    juce::Justification::centred);
        g.drawText (juce::String (gr, 1), meterArea.getX() - 4, meterArea.getY() - 13,
                    meterArea.getWidth() + 8, 12, juce::Justification::centred);
    }
