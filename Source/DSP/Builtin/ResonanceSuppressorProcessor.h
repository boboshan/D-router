...
public:
    ...
    void setStateInformation (const void* data, int size) override
    {
        ...
        if (auto* c = xml->getChildByName ("CURVE"))
            for (int i = 0; i < kNumNodes; ++i)
                nodes[(size_t) i].store ((float) c->getDoubleAttribute ("n" + juce::String (i), 0.0),
...
protected:
    ...
    void computeBinGains (const float* mags, float* gains, int numBins, double /*sr*/, int channel) override
    {
        ...
        if (resN != cachedResN) { rebuildDisplayBands (numBins, resN); cachedResN = resN; }
        ...
        if (gen != lastCurveGen) { rebuildCurveFromNodes (numBins); lastCurveGen = gen; }
        ...
        for (int k = 0; k < numBins; ++k)
        {
            ...
            if (k >= minBin)
            {
                ...
                targetRed = resonance::targetReductionDb (magDbWork[(size_t) k], baseWork[(size_t) k],
                                                          thr, sharp, maxRed);
            }
