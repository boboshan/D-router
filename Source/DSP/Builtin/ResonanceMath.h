...
struct NodeInterp { int lo; float frac; };
...
inline float targetReductionDb (float magDb, float baseDb, float thresholdDb,
                                float sharpness, float maxReductionDb) noexcept
{
...
inline float baseStrengthForRes (int n) noexcept
{
    switch (n)
    {
        ...
        case 6:  return 1.2f;
        case 12: return 2.4f;
        default: return 4.0f;   // 1/24 oct (and any unexpected value)
    }
}
