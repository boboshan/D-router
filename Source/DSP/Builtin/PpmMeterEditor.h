...
private:
    ...
    void drawScaleAndMeters (juce::Graphics& g)
    {
        ...
        auto yFor = [&] (float db) { return juce::jmap (juce::jlimit (lo, hi, db),
                                                        lo, hi, (float) bars.getBottom(), (float) bars.getY()); };
        ...
        for (float db = lo; db <= hi + 0.1f; db += 6.0f)
        {
            ...
            g.drawText (juce::String ((int) db), leftScale.getX(), y - 6, leftScale.getWidth() - 2, 12,
                        juce::Justification::centredRight);
            g.setColour (juce::Colour::fromRGB (90, 110, 150));
            g.drawText (juce::String ((int) (db - align)), rightScale.getX() + 2, y - 6, rightScale.getWidth() - 2, 12,
        ...
        for (int ch = 0; ch < n; ++ch)
        {
            ...
            juce::Rectangle<float> col ((float) bars.getX() + ch * bw + 2.0f, (float) bars.getY(),
                                        bw - 4.0f, (float) bars.getHeight());
            ...
            g.setFont (juce::FontOptions (9.0f));
            g.drawText (juce::String (hold, 1), (int) col.getX() - 2, bars.getBottom() + 1,
                        (int) col.getWidth() + 4, 12, juce::Justification::centred);
        }
    }
