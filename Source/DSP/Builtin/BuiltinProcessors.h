...
namespace dcr::builtin
{
    ...
    inline juce::dsp::IIR::Coefficients<float>::Ptr makeEqBandCoeffs (int type, double sr, float freq, float q, float gainDb)
    {
        const float g = std::pow (10.0f, gainDb * 0.05f);
        switch (type)
        {
            case 1:
                return juce::dsp::IIR::Coefficients<float>::makeLowShelf (sr, freq, q, g);
            case 2:
                return juce::dsp::IIR::Coefficients<float>::makeHighShelf (sr, freq, q, g);
            case 3:
            ...
            default:
                return juce::dsp::IIR::Coefficients<float>::makePeakFilter (sr, freq, q, g);
        }
    }
    ...
    public:
        ...
        BuiltinProcessor (juce::String identifier, juce::String displayName, APVTS::ParameterLayout layout)
        ...
        void prepareToPlay (double sr, int blockSize) override
        {
            ...
            preparedChannels = juce::jmax (2, getTotalNumInputChannels(), getTotalNumOutputChannels());
    ...
    protected:
        ...
        {
            if (auto* a = apvts.getRawParameterValue (pid))
                return a->load();
            return 0.0f;
        }
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            ...
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "gain", 1 }, "Gain", juce::NormalisableRange<float> (-60.0f, 24.0f, 0.1f), 0.0f, juce::AudioParameterFloatAttributes().withLabel ("dB")));
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            ...
            l.add (std::make_unique<juce::AudioParameterChoice> (
                juce::ParameterID { "type", 1 }, "Type", juce::StringArray { "High-pass", "Low-pass" }, 0));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "freq", 1 }, "Frequency", juce::NormalisableRange<float> (20.0f, 20000.0f, 1.0f, 0.25f), 100.0f, juce::AudioParameterFloatAttributes().withLabel ("Hz")));
            l.add (std::make_unique<juce::AudioParameterChoice> (
                juce::ParameterID { "slope", 1 }, "Slope", juce::StringArray { "12 dB/oct", "24 dB/oct", "36 dB/oct", "48 dB/oct" }, 1));
            return l;
        }
    ...
    protected:
        void prepareDsp (double sr, int, int numChannels) override
        {
            ...
            chans = numChannels;
            for (auto& s : stages)
                s.prepare (sr, numChannels);
            lastType = -1;
            lastFreq = -1.0f;
            lastSlope = -1; // force recompute
        }
    ...
    private:
        void updateCoeffsIfNeeded()
        {
            ...
            auto coeffsArray = (type == 0)
                                   ? juce::dsp::FilterDesign<float>::designIIRHighpassHighOrderButterworthMethod (freq, dspSampleRate, order)
                                   : juce::dsp::FilterDesign<float>::designIIRLowpassHighOrderButterworthMethod (freq, dspSampleRate, order);
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            ...
            for (int b = 0; b < kNumBands; ++b)
            {
                ...
                l.add (std::make_unique<juce::AudioParameterChoice> (
                    juce::ParameterID { p + "type", 1 }, label + "Type", juce::StringArray { "Bell", "Low Shelf", "High Shelf", "High-pass", "Low-pass" }, b == 0 ? 1 : (b == kNumBands - 1 ? 2 : 0)));
                l.add (std::make_unique<juce::AudioParameterFloat> (
                    juce::ParameterID { p + "freq", 1 }, label + "Freq", juce::NormalisableRange<float> (20.0f, 20000.0f, 1.0f, 0.25f), defFreqs[b], juce::AudioParameterFloatAttributes().withLabel ("Hz")));
                l.add (std::make_unique<juce::AudioParameterFloat> (
                    juce::ParameterID { p + "gain", 1 }, label + "Gain", juce::NormalisableRange<float> (-24.0f, 24.0f, 0.1f), 0.0f, juce::AudioParameterFloatAttributes().withLabel ("dB")));
                l.add (std::make_unique<juce::AudioParameterFloat> (
                    juce::ParameterID { p + "q", 1 }, label + "Q", juce::NormalisableRange<float> (0.1f, 10.0f, 0.01f, 0.3f), 0.707f));
            }
            return l;
        }
    ...
    protected:
        ...
        void processDsp (juce::AudioBuffer<float>& buffer) override
        {
            ...
            for (int b = 0; b < kNumBands; ++b)
            {
                if (readParam (ptrs[(size_t) b].on) < 0.5f)
                    continue;
    ...
    private:
        ...
        void updateBandIfNeeded (int b)
        {
            ...
            {
                case 1:
                    co = juce::dsp::IIR::Coefficients<float>::makeLowShelf (dspSampleRate, freq, q, gainLin);
                    break;
                case 2:
                    co = juce::dsp::IIR::Coefficients<float>::makeHighShelf (dspSampleRate, freq, q, gainLin);
                    break;
                case 3:
                    co = juce::dsp::IIR::Coefficients<float>::makeHighPass (dspSampleRate, freq, q);
                    break;
                case 4:
                    co = juce::dsp::IIR::Coefficients<float>::makeLowPass (dspSampleRate, freq, q);
                    break;
                default:
                    co = juce::dsp::IIR::Coefficients<float>::makePeakFilter (dspSampleRate, freq, q, gainLin);
                    break;
            }
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            APVTS::ParameterLayout l;
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "threshold", 1 }, "Threshold", juce::NormalisableRange<float> (-60.0f, 0.0f, 0.1f), -18.0f, juce::AudioParameterFloatAttributes().withLabel ("dB")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "ratio", 1 }, "Ratio", juce::NormalisableRange<float> (1.0f, 20.0f, 0.1f, 0.4f), 3.0f, juce::AudioParameterFloatAttributes().withLabel (": 1")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "attack", 1 }, "Attack", juce::NormalisableRange<float> (0.1f, 200.0f, 0.1f, 0.4f), 10.0f, juce::AudioParameterFloatAttributes().withLabel ("ms")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "release", 1 }, "Release", juce::NormalisableRange<float> (5.0f, 1000.0f, 1.0f, 0.4f), 120.0f, juce::AudioParameterFloatAttributes().withLabel ("ms")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "knee", 1 }, "Knee", juce::NormalisableRange<float> (0.0f, 24.0f, 0.1f), 6.0f, juce::AudioParameterFloatAttributes().withLabel ("dB")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "makeup", 1 }, "Makeup", juce::NormalisableRange<float> (-12.0f, 24.0f, 0.1f), 0.0f, juce::AudioParameterFloatAttributes().withLabel ("dB")));
            return l;
        }
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            APVTS::ParameterLayout l;
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "threshold", 1 }, "Threshold", juce::NormalisableRange<float> (-80.0f, 0.0f, 0.1f), -40.0f, juce::AudioParameterFloatAttributes().withLabel ("dB")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "attack", 1 }, "Attack", juce::NormalisableRange<float> (0.1f, 100.0f, 0.1f, 0.4f), 1.0f, juce::AudioParameterFloatAttributes().withLabel ("ms")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "hold", 1 }, "Hold", juce::NormalisableRange<float> (0.0f, 500.0f, 1.0f, 0.5f), 50.0f, juce::AudioParameterFloatAttributes().withLabel ("ms")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "release", 1 }, "Release", juce::NormalisableRange<float> (5.0f, 2000.0f, 1.0f, 0.4f), 200.0f, juce::AudioParameterFloatAttributes().withLabel ("ms")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "range", 1 }, "Range", juce::NormalisableRange<float> (-100.0f, 0.0f, 0.1f), -80.0f, juce::AudioParameterFloatAttributes().withLabel ("dB")));
            return l;
        }
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            APVTS::ParameterLayout l;
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "ceiling", 1 }, "Ceiling", juce::NormalisableRange<float> (-24.0f, 0.0f, 0.1f), -0.3f, juce::AudioParameterFloatAttributes().withLabel ("dB")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "release", 1 }, "Release", juce::NormalisableRange<float> (1.0f, 1000.0f, 1.0f, 0.4f), 100.0f, juce::AudioParameterFloatAttributes().withLabel ("ms")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "gain", 1 }, "Input Gain", juce::NormalisableRange<float> (0.0f, 24.0f, 0.1f), 0.0f, juce::AudioParameterFloatAttributes().withLabel ("dB")));
            return l;
        }
    ...
    protected:
        ...
        void processDsp (juce::AudioBuffer<float>& buffer) override
        {
            ...
            for (int i = 0; i < ns; ++i)
            {
                ...
                if (desired < gain)
                    gain = desired; // instant attack
                else
                    gain = relCoeff * gain + (1.0f - relCoeff) * 1.0f;
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            APVTS::ParameterLayout l;
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "size", 1 }, "Room Size", juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.5f));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "damping", 1 }, "Damping", juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.5f));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "wet", 1 }, "Wet", juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.33f));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "dry", 1 }, "Dry", juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 1.0f));
            return l;
        }
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            APVTS::ParameterLayout l;
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "time", 1 }, "Time", juce::NormalisableRange<float> (1.0f, 2000.0f, 1.0f, 0.4f), 300.0f, juce::AudioParameterFloatAttributes().withLabel ("ms")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "feedback", 1 }, "Feedback", juce::NormalisableRange<float> (0.0f, 0.95f, 0.01f), 0.35f));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "mix", 1 }, "Mix", juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.3f));
            return l;
        }
    ...
    protected:
        void prepareDsp (double sr, int blockSize, int numChannels) override
        {
            ...
            line.prepare ({ sr, (juce::uint32) juce::jmax (1, blockSize), (juce::uint32) juce::jmax (1, numChannels) });
            line.reset();
        }
        ...
        void processDsp (juce::AudioBuffer<float>& buffer) override
        {
            ...
            const float timeSamps = juce::jlimit (1.0f, (float) (maxDelaySamps - 1), param ("time") * 0.001f * (float) dspSampleRate);
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            ...
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "target", 1 }, "Target", juce::NormalisableRange<float> (-36.0f, 0.0f, 0.1f), -18.0f, juce::AudioParameterFloatAttributes().withLabel ("dB RMS")));
            ...
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "window", 1 }, "Window", juce::NormalisableRange<float> (50.0f, 5000.0f, 1.0f, 0.35f), 800.0f, juce::AudioParameterFloatAttributes().withLabel ("ms")));
            // Max amount the rider may boost or cut.
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "range", 1 }, "Range", juce::NormalisableRange<float> (0.0f, 24.0f, 0.1f), 12.0f, juce::AudioParameterFloatAttributes().withLabel ("+/- dB")));
            // How fast the applied gain itself moves (gain smoothing time).
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "smooth", 1 }, "Smooth", juce::NormalisableRange<float> (1.0f, 2000.0f, 1.0f, 0.4f), 400.0f, juce::AudioParameterFloatAttributes().withLabel ("ms")));
            // Below this measured level the rider freezes -- so pauses / noise
            // floor don't get ridden up.
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "gate", 1 }, "Gate", juce::NormalisableRange<float> (-80.0f, -20.0f, 0.1f), -50.0f, juce::AudioParameterFloatAttributes().withLabel ("dB")));
            return l;
        }
        ...
        void processDsp (juce::AudioBuffer<float>& buffer) override
        {
            ...
            for (int i = 0; i < ns; ++i)
            {
                ...
                if (measuredDb > gateDb)
                    gateOpen = true;
                else if (measuredDb < gateDb - 6.0f)
                    gateOpen = false;
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            ...
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "level", 1 }, "Level", juce::NormalisableRange<float> (-60.0f, 0.0f, 0.1f), -12.0f, juce::AudioParameterFloatAttributes().withLabel ("dB")));
            return l;
        }
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            APVTS::ParameterLayout l;
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "rate", 1 }, "Rate", juce::NormalisableRange<float> (0.1f, 20.0f, 0.01f, 0.4f), 5.0f, juce::AudioParameterFloatAttributes().withLabel ("Hz")));
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "depth", 1 }, "Depth", juce::NormalisableRange<float> (0.0f, 1.0f, 0.01f), 0.5f));
            l.add (std::make_unique<juce::AudioParameterChoice> (
                juce::ParameterID { "shape", 1 }, "Shape", juce::StringArray { "Sine", "Triangle", "Square" }, 0));
            return l;
        }
    ...
    protected:
        ...
        void processDsp (juce::AudioBuffer<float>& buffer) override
        {
            ...
            for (int i = 0; i < ns; ++i)
            {
                float lfo; // -1 .. 1
                if (shape == 1)
                    lfo = 4.0f * std::abs ((float) phase - 0.5f) - 1.0f; // triangle
                else if (shape == 2)
                    lfo = (phase < 0.5 ? 1.0f : -1.0f); // square
                else
                    lfo = std::sin ((float) phase * juce::MathConstants<float>::twoPi);
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            APVTS::ParameterLayout l;
            l.add (std::make_unique<juce::AudioParameterFloat> (
                juce::ParameterID { "width", 1 }, "Width", juce::NormalisableRange<float> (0.0f, 2.0f, 0.01f), 1.0f));
            return l;
        }
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            ...
            l.add (std::make_unique<Group> ("hp", "1  HP Filter", "|", std::make_unique<BoolP> (juce::ParameterID { "hp_on", 1 }, "HP On", false), std::make_unique<FloatP> (juce::ParameterID { "hp_freq", 1 }, "HP Freq", juce::NormalisableRange<float> (20.0f, 1000.0f, 1.0f, 0.3f), 80.0f, fa ("Hz"))));

            l.add (std::make_unique<Group> ("gate", "2  Gate", "|", std::make_unique<BoolP> (juce::ParameterID { "g_on", 1 }, "Gate On", false), std::make_unique<FloatP> (juce::ParameterID { "g_thresh", 1 }, "Gate Threshold", juce::NormalisableRange<float> (-80.0f, 0.0f, 0.1f), -40.0f, fa ("dB")), std::make_unique<FloatP> (juce::ParameterID { "g_range", 1 }, "Gate Range", juce::NormalisableRange<float> (-100.0f, 0.0f, 0.1f), -60.0f, fa ("dB")), std::make_unique<FloatP> (juce::ParameterID { "g_rel", 1 }, "Gate Release", juce::NormalisableRange<float> (5.0f, 2000.0f, 1.0f, 0.4f), 150.0f, fa ("ms"))));
            ...
            for (int b = 0; b < kEqBands; ++b)
            {
                ...
                eqGroup->addChild (
                    ...
                    std::make_unique<ChoiceP> (juce::ParameterID { eqId (b, "type"), 1 }, lab + "Type", juce::StringArray { "Bell", "Low Shelf", "High Shelf", "High-pass", "Low-pass" }, b == 0 ? 1 : (b == kEqBands - 1 ? 2 : 0)),
                    std::make_unique<FloatP> (juce::ParameterID { eqId (b, "freq"), 1 }, lab + "Freq", juce::NormalisableRange<float> (20.0f, 20000.0f, 1.0f, 0.25f), defFreqs[b], fa ("Hz")),
                    std::make_unique<FloatP> (juce::ParameterID { eqId (b, "gain"), 1 }, lab + "Gain", juce::NormalisableRange<float> (-18.0f, 18.0f, 0.1f), 0.0f, fa ("dB")),
                    std::make_unique<FloatP> (juce::ParameterID { eqId (b, "q"), 1 }, lab + "Q", juce::NormalisableRange<float> (0.1f, 10.0f, 0.01f, 0.3f), 0.707f));
            }
            l.add (std::move (eqGroup));

            l.add (std::make_unique<Group> ("comp", "4  Compressor", "|", std::make_unique<BoolP> (juce::ParameterID { "c_on", 1 }, "Comp On", false), std::make_unique<FloatP> (juce::ParameterID { "c_thresh", 1 }, "Comp Threshold", juce::NormalisableRange<float> (-60.0f, 0.0f, 0.1f), -18.0f, fa ("dB")), std::make_unique<FloatP> (juce::ParameterID { "c_ratio", 1 }, "Comp Ratio", juce::NormalisableRange<float> (1.0f, 20.0f, 0.1f, 0.4f), 3.0f, fa (": 1")), std::make_unique<FloatP> (juce::ParameterID { "c_attack", 1 }, "Comp Attack", juce::NormalisableRange<float> (0.1f, 200.0f, 0.1f, 0.4f), 10.0f, fa ("ms")), std::make_unique<FloatP> (juce::ParameterID { "c_rel", 1 }, "Comp Release", juce::NormalisableRange<float> (5.0f, 1000.0f, 1.0f, 0.4f), 120.0f, fa ("ms")), std::make_unique<FloatP> (juce::ParameterID { "c_makeup", 1 }, "Comp Makeup", juce::NormalisableRange<float> (-12.0f, 24.0f, 0.1f), 0.0f, fa ("dB"))));

            l.add (std::make_unique<Group> ("out", "5  Output", "|", std::make_unique<FloatP> (juce::ParameterID { "out_gain", 1 }, "Output Gain", juce::NormalisableRange<float> (-24.0f, 24.0f, 0.1f), 0.0f, fa ("dB"))));
    ...
    protected:
        ...
        void processDsp (juce::AudioBuffer<float>& buffer) override
        {
            ...
            if (param ("hp_on") > 0.5f)
            {
                ...
                if (hpf.hasCoefficients())
                    hpf.process (buffer, ns);
            }
            ...
            for (int b = 0; b < kEqBands; ++b)
            {
                ...
                if (eqBands[(size_t) b].hasCoefficients())
                    eqBands[(size_t) b].process (buffer, ns);
            }
    ...
    public:
        ...
        static APVTS::ParameterLayout createLayout()
        {
            ...
            l.add (std::make_unique<Group> ("xo", "Crossovers", "|", std::make_unique<FloatP> (juce::ParameterID { "x1", 1 }, "Low / Low-Mid", juce::NormalisableRange<float> (40.0f, 500.0f, 1.0f, 0.4f), 120.0f, fa ("Hz")), std::make_unique<FloatP> (juce::ParameterID { "x2", 1 }, "Low-Mid / High-Mid", juce::NormalisableRange<float> (200.0f, 4000.0f, 1.0f, 0.4f), 1000.0f, fa ("Hz")), std::make_unique<FloatP> (juce::ParameterID { "x3", 1 }, "High-Mid / High", juce::NormalisableRange<float> (2000.0f, 16000.0f, 1.0f, 0.4f), 6000.0f, fa ("Hz"))));

            const char* bandNames[kBands] = { "Low", "Low-Mid", "High-Mid", "High" };
            for (int b = 0; b < kBands; ++b)
            {
                const juce::String gname = juce::String (b + 1) + "  Band: " + bandNames[b];
                l.add (std::make_unique<Group> (bandId (b, "grp").toStdString(), gname.toStdString(), "|", std::make_unique<BoolP> (juce::ParameterID { bandId (b, "on"), 1 }, "On", true), std::make_unique<FloatP> (juce::ParameterID { bandId (b, "thresh"), 1 }, "Threshold", juce::NormalisableRange<float> (-60.0f, 0.0f, 0.1f), -18.0f, fa ("dB")), std::make_unique<FloatP> (juce::ParameterID { bandId (b, "ratio"), 1 }, "Ratio", juce::NormalisableRange<float> (1.0f, 20.0f, 0.1f, 0.4f), 3.0f, fa (": 1")), std::make_unique<FloatP> (juce::ParameterID { bandId (b, "attack"), 1 }, "Attack", juce::NormalisableRange<float> (0.1f, 200.0f, 0.1f, 0.4f), 10.0f, fa ("ms")), std::make_unique<FloatP> (juce::ParameterID { bandId (b, "rel"), 1 }, "Release", juce::NormalisableRange<float> (5.0f, 1000.0f, 1.0f, 0.4f), 120.0f, fa ("ms")), std::make_unique<FloatP> (juce::ParameterID { bandId (b, "makeup"), 1 }, "Makeup", juce::NormalisableRange<float> (-12.0f, 24.0f, 0.1f), 0.0f, fa ("dB"))));
            }

            l.add (std::make_unique<Group> ("out", "Output", "|", std::make_unique<FloatP> (juce::ParameterID { "out_gain", 1 }, "Output Gain", juce::NormalisableRange<float> (-24.0f, 24.0f, 0.1f), 0.0f, fa ("dB"))));
    ...
    protected:
        void prepareDsp (double sr, int blockSize, int numChannels) override
        {
            dspSampleRate = sr;
            chans = numChannels;
            juce::dsp::ProcessSpec spec { sr, (juce::uint32) juce::jmax (1, blockSize), (juce::uint32) juce::jmax (1, numChannels) };
            lr1.prepare (spec);
            lr2.prepare (spec);
            lr3.prepare (spec);
            for (int b = 0; b < kBands; ++b)
        ...
        void processDsp (juce::AudioBuffer<float>& buffer) override
        {
            ...
            struct BP
            {
                float thr, ratio, makeup, attCoeff, relCoeff;
                bool on;
            };
            BP bp[kBands];
            for (int b = 0; b < kBands; ++b)
            ...
            for (int i = 0; i < ns; ++i)
            {
                ...
                for (int b = 0; b < kBands; ++b)
                {
                    ...
                    if (bp[b].on)
                    {
                        ...
                        if (over > 0.0f)
                            grDb = -(over * (1.0f - 1.0f / bp[b].ratio));
