...
public:
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        for (int i = 0; i < n; ++i) spec.lineTo (xFor (i), yMag (proc.getDisplayMagDb (i)));
        spec.lineTo (R, B);
        spec.closeSubPath();
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            if (i == 0) gainPath.startNewSubPath (x, y); else gainPath.lineTo (x, y);
        }
        ...
        g.drawText ("spectrum + correction (low -> high)", a.getX() + 3, a.getY() + 2,
                    260, 12, juce::Justification::topLeft);
    }
