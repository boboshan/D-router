...
public:
    ...
    void resized() override
    {
        ...
        generic->setSize (r.getWidth() - 10, juce::jmax (generic->getHeight(),
                          (2 + 3 + ChannelStripProcessor::kEqBands * 5 + 6 + 1) * 26 + 60));
    }
    ...
    void mouseDrag (const juce::MouseEvent& e) override
    {
        ...
        setParam (ChannelStripProcessor::eqId (dragBand, "freq"),
                  juce::jlimit (20.0f, 20000.0f,
                      freqForX (juce::jlimit ((float) curveArea.getX(), (float) curveArea.getRight(), e.position.x))));
        ...
        if (type <= 2)
            setParam (ChannelStripProcessor::eqId (dragBand, "gain"),
                      juce::jlimit (-18.0f, 18.0f,
                          gainForY (juce::jlimit ((float) curveArea.getY(), (float) curveArea.getBottom(), e.position.y))));
...
private:
    ...
    {
        if (auto* a = strip.getValueTreeState().getRawParameterValue (id)) return a->load();
        return 0.0f;
    }
    ...
    {
        return std::pow (10.0f, juce::jmap (x, (float) curveArea.getX(), (float) curveArea.getRight(),
                                            std::log10 (20.0f), std::log10 (20000.0f)));
    }
    float yForGain (float db) const
    {
        return juce::jmap (juce::jlimit (-kSpan, kSpan, db), kSpan, -kSpan,
                           (float) curveArea.getY(), (float) curveArea.getBottom());
    }
    ...
    {
        for (int b = 0; b < ChannelStripProcessor::kEqBands; ++b)
            if (nodePos (b).getDistanceFrom (p) < 12.0f) return b;
        return -1;
    }
    ...
    void drawCurve (juce::Graphics& g)
    {
        ...
        for (float f : { 50.0f, 100.0f, 500.0f, 1000.0f, 5000.0f, 10000.0f })
        {
            ...
            g.drawText (f >= 1000.0f ? juce::String (f / 1000.0f, 0) + "k" : juce::String ((int) f),
                        (int) x + 2, curveArea.getBottom() - 13, 36, 12, juce::Justification::topLeft);
            g.setColour (juce::Colour::fromRGB (40, 40, 48));
        }
        ...
        for (int x = curveArea.getX(); x <= curveArea.getRight(); ++x)
        {
            ...
            if (x == curveArea.getX()) path.startNewSubPath ((float) x, y);
    ...
    void drawMeter (juce::Graphics& g)
    {
        ...
        g.drawText ("Comp GR", meterArea.getX() - 8, meterArea.getBottom() + 1, meterArea.getWidth() + 16, 12,
