...
public:
    ...
    static APVTS::ParameterLayout createLayout()
    {
        ...
        l.add (std::make_unique<juce::AudioParameterChoice>(
            juce::ParameterID { "standard", 1 }, "Standard",
            juce::StringArray { "EBU (IEC IIb)", "BBC (IEC IIa)", "DIN 45406",
                                "Nordic", "Digital peak" }, 0));
        // dBFS that corresponds to 0 dBu / alignment level, for the dBu scale.
        l.add (std::make_unique<juce::AudioParameterFloat>(
            juce::ParameterID { "align", 1 }, "Align 0dBu",
            juce::NormalisableRange<float> (-24.0f, -9.0f, 0.1f), -18.0f,
...
protected:
    struct Standard { const char* name; float attackMs; float fallbackDbPerSec; };
    ...
    {
        dspSampleRate = sr;
        for (auto& v : qp)     v = 0.0f;
        for (auto& v : hold)   v = 0.0f;
        for (auto& v : qpDb)   v.store (-100.0f, std::memory_order_relaxed);
        for (auto& v : holdDb) v.store (-100.0f, std::memory_order_relaxed);
        holdCounter.fill (0);
    }
    ...
    void processDsp (juce::AudioBuffer<float>& buffer) override
    {
        ...
        for (int ch = 0; ch < nch; ++ch)
        {
            ...
            for (int i = 0; i < ns; ++i)
            {
                ...
                if (r > env) env += (r - env) * attackCoeff;   // integrate up
                else         env *= decayMul;                   // PPM fallback

                if (env >= peak) { peak = env; hc = holdSamples; }
                else if (--hc <= 0) { peak *= decayMul; hc = 0; }
            }
