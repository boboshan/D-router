...
namespace dcr
{
    ...
    {
        if (ch < 0 || ch >= (int) channelToGroupIdx.size())
            return -1;
    ...
    void OutputGroupManager::assignChannel (int groupIdx, int slotIdx, int globalOutputCh)
    {
        ...
        if (slotIdx < 0 || slotIdx >= (int) g.memberChannels.size())
            return;
        ...
        if (globalOutputCh >= 0 && globalOutputCh < numOutputs)
        {
            ...
            if (prevGroup >= 0)
            {
                ...
                for (auto& m : prev.memberChannels)
                    if (m == globalOutputCh)
                        m = -1;
            }
        }
    ...
    void OutputGroupManager::setGroupFaderMode (int groupIdx, OutputGroup::FaderMode mode, RoutingMatrix& matrix)
    {
        if (groupIdx < 0 || groupIdx >= (int) groups.size())
            return;
        auto& g = *groups[(size_t) groupIdx];
        if (g.faderMode.load (std::memory_order_relaxed) == mode)
            return;
    ...
    void OutputGroupManager::addGroupInsertLatencySamples (std::vector<int>& perCh) const
    {
        ...
        for (auto& g : groups)
        {
            ...
            for (auto& slot : g->pluginSlots)
                if (slot != nullptr)
                    groupLat += slot->getChainLatencySamples();
            if (groupLat <= 0)
                continue;
