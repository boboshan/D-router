...
namespace dcr
{
        ...
        const EngineSettings& settings)
    {
        ...
        for (auto& d : outputDelays)
            d.prepare (kMaxPdcSamples, blockSize);
    ...
    float MatrixProcessor::getInputPeak (int n) const noexcept
    {
        return (n >= 0 && n < (int) inputPeaks.size())
                   ? inputPeaks[(size_t) n].load (std::memory_order_relaxed)
                   : 0.0f;
    }
    float MatrixProcessor::getOutputPeak (int m) const noexcept
    {
        return (m >= 0 && m < (int) outputPeaks.size())
                   ? outputPeaks[(size_t) m].load (std::memory_order_relaxed)
                   : 0.0f;
    }
    ...
    void MatrixProcessor::refreshSnapshotIfDirty()
    {
        ...
        for (int n = 0; n < nIn; ++n)
        {
            if (matrix->getInputMute (n))
                continue;
            if (anySolo && !matrix->getInputSolo (n))
                continue;
            const float groupG = inputGroupManager != nullptr
                                     ? inputGroupManager->getChannelRouterGain (n)
                                     : 1.0f;
    ...
    bool MatrixProcessor::tryProcessOneBlock()
    {
        ...
        for (auto& rt : activeRoutes)
        {
            ...
            if (rt.currentGain <= 1.0e-7f && rt.targetGain == 0.0f)
                continue;
